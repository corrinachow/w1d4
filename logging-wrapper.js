var wrapLog = function (callback, name) {
  return function (...numbers) {
    if (numbers.length === 2) {
      const totalArea = callback.apply(null, numbers);
      return `${name}(${numbers.join(', ')}) => ${totalArea}`
    } else if (numbers.length === 3) {
      const totalVol = callback.apply(null, numbers);
      return `${name}(${numbers.join(', ')}) => ${totalVol}`
    };
}
};
